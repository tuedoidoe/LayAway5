...
# ==========================================
# CARREGAMENTO DOS DICIONÃRIOS JSON
# ==========================================
@st.cache_data # Desativar, fazer as modificaÃ§Ãµes no arquivo mapeamentos.json, rodar, ativar novamente e depois ativar.
...
def identificar_torneio(nome_sujo):
    ...
    return nome_sujo
# ==========================================
# CONFIGURAÃ‡ÃƒO DA PÃGINA E LOGIN
# ==========================================
...
st.markdown("""
    ...
    /* ConfiguraÃ§Ã£o Geral de BotÃµes (PrimÃ¡rio Verde, SecundÃ¡rio Cinza) */
...
def check_password():
    ...
    with col2:
        ...
        st.text_input("ðŸ”‘ Senha:", type="password", on_change=password_entered, key="password")
    ...
    return False
# ==========================================
# FUNÃ‡Ã•ES DE CARREGAMENTO (COM CACHE) E GRÃFICO
# ==========================================
...
@st.dialog("ðŸ“Š Raio-X do Confronto: Boca de JacarÃ©", width="large")
def abrir_popup_grafico(t_casa, t_fora, df_completo):
    ...
    if not hist_casa.empty:
        fig.add_trace(go.Scatter(
            ...
            mode='lines+markers', name=f"ðŸ“ˆ Poder de Fogo ({t_casa})",
    ...
    if not hist_fora.empty:
        fig.add_trace(go.Scatter(
            ...
            mode='lines+markers', name=f"ðŸ“‰ Crise Defensiva ({t_fora})",
    ...
    fig.update_layout(
        ...
        yaxis=dict(title='MÃ©dia MÃ³vel de Gols', showgrid=True, gridwidth=1, gridcolor='#333', range=[-0.5, teto_grafico]),
        hovermode="x unified",
    ...
    with col_msg1:
        if momento_ataque_casa >= 1.5:
            st.success(f"ðŸ”¥ **Ataque Feroz:** A tendÃªncia atual do {t_casa} Ã© marcar {momento_ataque_casa:.1f} gols/jogo.")
        elif momento_ataque_casa >= 1.0:
            st.info(f"âš–ï¸ **Ataque Regular:** A tendÃªncia atual do {t_casa} Ã© marcar {momento_ataque_casa:.1f} gols/jogo.")
        else:
            st.error(f"âš ï¸ **Ataque Inofensivo:** O {t_casa} vem sofrendo para marcar (tendÃªncia de {momento_ataque_casa:.1f} gols/jogo).")
    with col_msg2:
        if momento_defesa_fora >= 1.5:
            st.success(f"ðŸš¨ **Defesa em Crise:** A zaga do {t_fora} estÃ¡ vazando! Sofrendo {momento_defesa_fora:.1f} gols/jogo.")
        elif momento_defesa_fora >= 1.0:
            st.info(f"âš–ï¸ **Defesa Regular:** A tendÃªncia atual do {t_fora} Ã© sofrer {momento_defesa_fora:.1f} gols/jogo.")
        else:
            st.error(f"ðŸ›¡ï¸ **Defesa IntransponÃ­vel:** O {t_fora} ajustou a zaga e vem sofrendo apenas {momento_defesa_fora:.1f} gols/jogo.")
    if momento_ataque_casa >= 1.5 and momento_defesa_fora >= 1.5:
        veredito = "ðŸŠ <b>BOCA DE JACARÃ‰ DETECTADA:</b> CenÃ¡rio PERFEITO para Lay Away! O ataque do mandante estÃ¡ crescendo na mesma proporÃ§Ã£o em que a defesa do visitante estÃ¡ afundando."
        cor_borda = "#00d26a"
    elif momento_ataque_casa < 1.0 and momento_defesa_fora < 1.0:
        veredito = "ðŸ§± <b>CENÃRIO PERIGOSO:</b> Mandante com ataque inoperante contra uma defesa visitante ajustada. Risco alto de jogo truncado ou 0x0. Evite o Lay Away."
        cor_borda = "#ff4b4b"
    elif momento_ataque_casa >= 1.5 and momento_defesa_fora < 1.0:
        veredito = "âš”ï¸ <b>JOGO DE PACIÃŠNCIA:</b> O mandante tem muito volume, mas o visitante sabe se defender. A linha de gols dependerÃ¡ da quebra dessa retranca."
        cor_borda = "#fada5e"
    elif momento_ataque_casa < 1.0 and momento_defesa_fora >= 1.5:
        veredito = "ðŸŽ² <b>CENÃRIO IMPREVISÃVEL:</b> A defesa visitante Ã© terrÃ­vel, mas o ataque mandante nÃ£o aproveita. Jogo com alta chance de zebras."
        cor_borda = "#fada5e"
    else:
        veredito = "âš–ï¸ <b>CENÃRIO NEUTRO:</b> As mÃ©dias mÃ³veis estÃ£o estÃ¡veis. NÃ£o hÃ¡ padrÃ£o claro de Boca de JacarÃ© neste momento."
        cor_borda = "#3b82f6"
    st.markdown(f"""
    ...
    with st.spinner(st_context_msg):
        try:
            ...
            if not df_livescore.empty:
                ...
                # A base livescore inteira é a fonte histórica estável. Não a
                # recorte pelo início do intervalo: isso fazia a mesma partida
                # receber números diferentes em "Data Única" e "Intervalo".
            ...
            # A hora evita que a ordem alfabética do mandante defina o que é
            # considerado passado quando há mais de um jogo no mesmo dia.
            ...
            # Jogos-alvo não têm placar. Eles precisam permanecer como NaN;
            # tratá-los como 0 aqui os transformava em derrotas fictícias e
            # contaminava as janelas de cinco jogos de todo o intervalo.
            ...
            df_stats['Pts_H'] = np.select(
                ...
                [3.0, 1.0], default=np.nan
            )
            df_stats['Pts_A'] = np.select(
                [jogo_finalizado & (df_stats['Goals_A_FT'] > df_stats['Goals_H_FT']), jogo_finalizado & (df_stats['Goals_H_FT'] == df_stats['Goals_A_FT'])],
                [3.0, 1.0], default=np.nan
            )
            ...
            df_stats['Is_CS_Casa'] = np.where(jogo_finalizado, (df_stats['Goals_A_FT'] == 0).astype(float), np.nan)
            df_stats['Is_FTS_Fora'] = np.where(jogo_finalizado, (df_stats['Goals_A_FT'] == 0).astype(float), np.nan)
...
def pagina_scanner():
    ...
    agora = datetime.now(fuso_br).strftime("%d/%m/%Y Ã s %H:%M:%S")
    st.markdown(f"<p class='data-atualizacao'>Ãšltima atualizaÃ§Ã£o: {agora}</p>", unsafe_allow_html=True)
    col_nav, espaco, col_rad, col_dat, col_btn_pesq = st.columns([1.2, 1.5, 1.4, 1.2, 1.2])
    with col_nav:
        st.markdown("<div style='margin-top: 28px;'></div>", unsafe_allow_html=True)
        if st.button("ðŸ“Š Ver Resultados Passados", key="btn_nav_res", use_container_width=True, type="secondary"):
            mudar_pagina('resultados')
            st.rerun()
    with col_rad:
        st.markdown("<div style='margin-top: 30px;'></div>", unsafe_allow_html=True)
        tipo_filtro = st.radio("PerÃ­odo", ["Data Ãšnica", "Intervalo"], horizontal=True, label_visibility="collapsed", key="scan_rad")
    with col_dat:
    ...
    with col_btn_pesq:
        st.markdown("<div style='margin-top: 28px;'></div>", unsafe_allow_html=True)
        btn_procurar = st.button("ðŸš€ Iniciar Varredura", use_container_width=True, key="scan_btn", type="primary")
    ...
    if btn_procurar:
        ...
        if df_bruto.empty:
            texto_data = data_selecionada.strftime('%d/%m/%Y') if tipo_filtro == "Data Ãšnica" else "no perÃ­odo selecionado"
            st.warning(f"O modelo nÃ£o encontrou Edge suficiente (>0.0%) ou nÃ£o hÃ¡ jogos qualificados para {texto_data}.")
        else:
    ...
    if st.session_state.get('mostrar_tabela_scanner', False):
        ...
        with col_sort: coluna_ordem = st.selectbox("Ordenar por", ["HorÃ¡rio", "EV+", "Score"], key="scan_ord1")
        ...
        if not tabela.empty:
            ...
            if coluna_ordem == "HorÃ¡rio": tabela = drop_reset_index(tabela.sort_values(by=['Date', 'Time'], ascending=[is_ascending, is_ascending]))
            ...
            tabela_excel = tabela.rename(columns={'Date': 'Data', 'Time': 'HorÃ¡rio', 'League': 'Liga', 'Home': 'Time Casa', 'Away': 'Time Fora', 'Odd_A_Lay': 'Odd Lay', 'Pontos Casa': 'Pts Casa', 'Pontos Fora': 'Pts Fora', 'XG_Casa': 'xG Casa', 'XG_Fora': 'xG Fora', 'Edge': 'EV+'})
            buffer = io.BytesIO()
            with pd.ExcelWriter(buffer, engine='openpyxl') as writer: tabela_excel.to_excel(writer, index=False, sheet_name='Lay_Away')
            with col_btn:
                st.markdown("<div style='margin-top: 28px;'></div>", unsafe_allow_html=True)
                st.download_button("ðŸ“¥ Exportar Excel", data=buffer.getvalue(), file_name="Jogos_LayAway.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=False)
            ...
            tooltips_dicionario = {
                '>xG Casa</th>': '><span class="tooltip-header" data-title="A Verdade Atual: O diferencial entre o xG do Mandante e do Visitante dita o favoritismo real de hoje. Ã‰ o motor do modelo. | Quanto MAIOR, melhor. (Ideal: > 1.50)">xG Casa</span></th>',
                '>xG Fora</th>': '><span class="tooltip-header" data-title="A Verdade Atual: O diferencial entre o xG do Mandante e do Visitante dita o favoritismo real de hoje. Ã‰ o motor do modelo. | Quanto MENOR, melhor. (Ideal: < 1.00)">xG Fora</span></th>',
                '>Pts Casa</th>': '><span class="tooltip-header" data-title="Soma os pontos do time jogando em casa nos Ãºltimos 5 jogos. | Quanto MAIOR, melhor. (Ideal: >= 10 pts)">Pts Casa</span></th>',
                '>Pts Fora</th>': '><span class="tooltip-header" data-title="Soma os pontos do time jogando fora de casa nos Ãºltimos 5 jogos. | Quanto MENOR, melhor. (Ideal: <= 5 pts)">Pts Fora</span></th>',
                '>FTS Fora</th>': '><span class="tooltip-header" data-title="Quantas vezes o time visitante nÃ£o marcou nenhum gol nos seus Ãºltimos 5 jogos fora de casa e tira a mÃ©dia. | Quanto MAIOR, melhor. (Ideal: >= 0.60)">FTS Fora</span></th>',
                '>DP GM Fora</th>': '><span class="tooltip-header" data-title="(Desvio PadrÃ£o Gols Marcados) Calcula a oscilaÃ§Ã£o de gols marcados pelo visitante nos Ãºltimos 5 jogos. | Quanto MENOR, melhor. (Ideal: < 1.00)">DP GM Fora</span></th>',
                '>DP GS Casa</th>': '><span class="tooltip-header" data-title="(Desvio PadrÃ£o Gols Sofridos) A mesma lÃ³gica do desvio padrÃ£o, mas aplicada aos gols que o mandante sofreu nos Ãºltimos 5 jogos em casa, para medir a estabilidade da zaga. | Quanto MENOR, melhor. (Ideal: < 1.00)">DP GS Casa</span></th>',
                '>Vaz Def Fora</th>': '><span class="tooltip-header" data-title="Pega a mÃ©dia de gols sofridos pelo time visitante nos Ãºltimos 5 jogos fora de casa. | Quanto MAIOR, melhor. (Ideal: >= 1.50)">Vaz Def Fora</span></th>',
                '>CS Casa</th>': '><span class="tooltip-header" data-title="Verifica quantas vezes o time da casa nÃ£o sofreu gols nos seus Ãºltimos 5 jogos em casa e tira a mÃ©dia. | Quanto MAIOR, melhor. (Ideal: >= 0.40)">CS Casa</span></th>',
                '>EV+</th>': '><span class="tooltip-header" data-title="Vantagem (Edge): Margem de valor real encontrada pelo modelo em relaÃ§Ã£o Ã  cotaÃ§Ã£o da casa de apostas. | Quanto MAIOR, melhor.">EV+</span></th>',
                '>Score</th>': '><span class="tooltip-header" data-title="Nota de 0 a 100 gerada pela normalizaÃ§Ã£o de todos os pesos. Serve como um guia de risco consolidado. | Quanto MAIOR, melhor. (Ideal: >= 55)">Score</span></th>',
                '>Alerta</th>': '><span class="tooltip-header" data-title="VisualizaÃ§Ã£o RÃ¡pida de Risco. Verde >= 55. Amarelo 48 a 54. Vermelho < 48. | O ideal Ã© focar nos Verdes e Amarelos.">Alerta</span></th>'
            }
            ...
            st.markdown("<p style='font-size: 16px; color: #e0e0e0; font-weight: bold; margin-bottom: 5px;'>ðŸ“Š Abrir GrÃ¡fico de TendÃªncia (Pop-up)</p>", unsafe_allow_html=True)
            ...
            with col_btn_graf:
                ...
                if st.button("ðŸ“ˆ Ver GrÃ¡fico na Janela", use_container_width=True, key="scan_btn_graf", type="secondary"):
                    if jogo_alvo:
                        t_casa, t_fora = jogo_alvo.split(" x ")
                        abrir_popup_grafico(t_casa, t_fora, df_completo)
        else:
            st.info("Nenhum jogo atende aos critÃ©rios do modelo para a data selecionada.")
# ==========================================
# PÃGINA 2: RESULTADOS PASSADOS
# ==========================================
def pagina_resultados():
    ...
    with col_nav:
        st.markdown("<div style='margin-top: 28px;'></div>", unsafe_allow_html=True)
        if st.button("ðŸ”™ Voltar para o Scanner", key="btn_nav_scan", use_container_width=True, type="secondary"):
            mudar_pagina('scanner')
            st.rerun()
    with col_rad:
        st.markdown("<div style='margin-top: 30px;'></div>", unsafe_allow_html=True)
        tipo_filtro = st.radio("PerÃ­odo", ["Data Ãšnica", "Intervalo"], horizontal=True, label_visibility="collapsed", key="res_rad")
    with col_dat:
        st.markdown("<div style='font-size: 14px; font-weight: bold; margin-bottom: 2px;'>Data da Pesquisa</div>", unsafe_allow_html=True)
    ...
    with col_btn_pesq:
        st.markdown("<div style='margin-top: 28px;'></div>", unsafe_allow_html=True)
        btn_procurar = st.button("ðŸš€ Iniciar Pesquisa", use_container_width=True, key="res_btn", type="primary")
    st.markdown("<hr style='margin-top: 0px; margin-bottom: 25px; border: 1px solid #333;'>", unsafe_allow_html=True)
    if btn_procurar:
        st.session_state['mostrar_tabela_resultados'] = False 
        df_bruto, _, df_livescore = rodar_engine_pesquisa(data_selecionada, tipo_filtro, "Calculando resultados histÃ³ricos...")
        if df_bruto.empty:
            st.warning("NÃ£o foram encontrados alertas do Scanner no perÃ­odo selecionado.")
        else:
            ...
            if df_final.empty:
                st.info("Os jogos alertados pelo scanner nesta data ainda nÃ£o possuem placar finalizado no banco de dados.")
            else:
    ...
    if st.session_state.get('mostrar_tabela_resultados', False):
        ...
        if df_view.empty:
            ...
            # --- NOVOS CÃLCULOS DE GREEN E RED ---
            ...
            with col_res1:
                ...
                <div style='text-align: left; font-size: 18px; margin-top: 34px; margin-bottom: 20px;'>
                    <span style='color: #888;'>OperaÃ§Ãµes Finalizadas:</span> <span style='color: #00d26a; font-weight: 900;'>{len(df_view)}</span> | 
                    <span style='color: #888;'>Resultado PerÃ­odo:</span> <span style='color: {cor_lucro}; font-weight: 900;'>R$ {lucro_total:.2f}</span> | 
                    <span style='color: #888;'>Odd MÃ©dia PerÃ­odo:</span> <span style='color: #e0e0e0; font-weight: bold;'>{odd_media:.2f}</span> | 
            ...
            with col_btn:
                st.markdown("<div style='margin-top: 28px;'></div>", unsafe_allow_html=True)
                st.download_button("ðŸ“¥ Exportar Resultados Excel", data=buffer.getvalue(), file_name="Resultados Jogos Lay Away.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True, key="res_export")
